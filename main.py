...
class BST:
    def insert(self, root, key, data):
        if root is None:
            return Node(key, data) # Si no hay raíz, se crea un nuevo nodo como raíz
        if key < root.key:
            root.left = self.insert(root.left, key, data)# Si la clave es menor, se inserta en el subárbol izquierdo (recursivamente)
        else:
            root.right = self.insert(root.right, key, data) # Si la clave es mayor o igual, se inserta en el subárbol derecho (recursivamente)
    ...
    def search(self, root, key):# Busca un nodo con la clave dada
        ...
        if key < root.key:
            return self.search(root.left, key) # Si la clave es menor, se busca en el subárbol izquierdo (recursivamente)
        return self.search(root.right, key)  # Si la clave es mayor, se busca en el subárbol derecho (recursivamente)

    def delete(self, root, key):
        if root is None:
            return root # Si la raíz es nula, no hay nada que borrar
        if key < root.key:
            root.left = self.delete(root.left, key)# Si la clave es menor, se borra en el subárbol izquierdo
        elif key > root.key:
            root.right = self.delete(root.right, key)# Si la clave es mayor, se borra en el subárbol derecho
        else:
            ...
            temp = self.minValueNode(root.right)# Se busca el nodo con el valor mínimo en el subárbol derecho
            root.key = temp.key# Se reemplaza el valor de la raíz con el valor mínimo encontrado
            root.data = temp.data# Se reemplaza el dato de la raíz con el dato del valor mínimo encontrado
            root.right = self.delete(root.right, temp.key)# Se elimina el nodo con el valor mínimo encontrado
        return root# Se devuelve la raíz modificada
    ...
    def minValueNode(self, node):
        ...
        return current# Devuelve el nodo más a la izquierda (el que tiene el valor mínimo)
...
class AVLNode:
    def __init__(self, key, data): 
        self.key = key# El valor clave del nodo (usado para ordenar en el AVL)
        self.data = data# La información que almacena el nodo
        self.height = 1# La altura del nodo (inicialmente 1, ya que es un nodo hoja)
        self.left = None# Referencia al hijo izquierdo (inicialmente vacío)
        self.right = None# Referencia al hijo derecho (inicialmente vacío)
...
class AVL:
    def getHeight(self, root):
        if not root:
            return 0
        return root.height #Calcula la altura de un nodo dado. Si el nodo es None, devuelve 0 (caso base para nodos vacíos).
    ...
    def rightRotate(self, y):
        ...
        return x #Realiza una rotación a la derecha sobre el nodo y. Esto implica reordenar los nodos x, T2 e y, y actualizar sus alturas.
    ...
    def leftRotate(self, x):
        ...
        x.height = 1 + max(self.getHeight(x.left), self.getHeight(x.right))
        y.height = 1 + max(self.getHeight(y.left), self.getHeight(y.right))
        return y #Realiza una rotación a la izquierda sobre el nodo x, similar a la rotación derecha pero en sentido opuesto.

    def getBalance(self, root):
        if not root:
            return 0
        return self.getHeight(root.left) - self.getHeight(root.right)#Calcula el factor de equilibrio de un nodo, que es la diferencia 
...
#de equilibrio y realiza rotaciones (si es necesario) para mantener el árbol balanceado.
    ...
    def search(self, root, key):
        ...
        return self.search(root.right, key)#Busca un nodo con la clave dada en el árbol AVL. La lógica es idéntica a la búsqueda en un BST, ya que la estructura básica del árbol es la misma.
...
class Graph:
    def __init__(self):
        self.graph = defaultdict(list)#El constructor inicializa un diccionario vacío llamado graph usando defaultdict(list). 
    ...
    def add_edge(self, u, v):
        self.graph[u].append(v)#Este método agrega una arista (conexión) entre los nodos u y v al grafo. 
        #Agrega v a la lista de nodos adyacentes de u. 

    def find_shortest_path(self, start, end, path=[]): #Este método encuentra el camino más corto entre los nodos start y end utilizando una búsqueda en profundidad (DFS) recursiva.
#Inicialmente, path es una lista vacía que se irá llenando con los nodos visitados en el camino.
        path = path + [start]
        if start == end:
            return path #Si el nodo start es igual al nodo end, se ha encontrado el destino y se devuelve el camino actual (path).
        if start not in self.graph:
            return None #Si el nodo start no está en el diccionario graph, significa que no tiene vecinos y no hay camino posible, por lo que se devuelve None.
        shortest = None
        for node in self.graph[start]: #En caso contrario, se itera sobre los nodos adyacentes de start
            if node not in path:
                newpath = self.find_shortest_path(node, end, path) #Si el nodo vecino aún no ha sido visitado (no está en path), se realiza una llamada recursiva a find_shortest_path para explorar ese camino.
                if newpath:
                    if not shortest or len(newpath) < len(shortest):
                        shortest = newpath #Si la llamada recursiva devuelve un camino válido (newpath), se compara su longitud con el camino más corto encontrado hasta el momento (shortest). Si es más corto, se actualiza shortest.
        return shortest #Finalmente, se devuelve el camino más corto encontrado (shortest).
...
class StudentManagementApp:
    ...
    def setup_course_ui(self, parent_frame):
        frame = tk.LabelFrame(parent_frame, text="Gestión de Cursos", padx=10, pady=10)
        frame.pack(padx=10, pady=10)
        ...
        tk.Button(frame, text="Agregar Curso", command=self.add_course).grid(row=2, column=0)
        tk.Button(frame, text="Buscar Curso", command=self.search_course).grid(row=2, column=1)

    def setup_dependency_ui(self, parent_frame):
        frame = tk.LabelFrame(parent_frame, text="Gestión de Dependencias", padx=10, pady=10)
        frame.pack(padx=10, pady=10)

        tk.Label(frame, text="Curso Prerrequisito").grid(row=0, column=0, sticky="w")
        tk.Label(frame, text="Curso Dependiente").grid(row=1, column=0, sticky="w")
        ...
        tk.Button(frame, text="Agregar Dependencia", command=self.add_dependency).grid(row=2, column=0)
        tk.Button(frame, text="Encontrar Ruta", command=self.find_path).grid(row=2, column=1)

    def add_student(self):
        student_id = int(self.student_id_entry.get())
        student_name = self.student_name_entry.get()
        self.bst_root = self.bst.insert(self.bst_root, student_id, student_name)
    ...
    def search_student(self):
        ...
        if result:
            messagebox.showinfo("Resultado", f"Estudiante encontrado: {result.data}")
        else:
    ...
    def search_course(self):
        ...
        if result:
            messagebox.showinfo("Resultado", f"Curso encontrado: {result.data}")
        else:
